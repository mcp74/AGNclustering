...
def wpmu_cross_jackknife(d1,d2,r1,r2,m,mumax,sbins,mubins,estimator,covariance=True,survey=None,weights1=None,weights2=None):
	#print('beginning jackknifes')
	if survey=='BASS':
		d1blocks=np.array(_BASS_block(d1,m),dtype=object)
		d2blocks=np.array(_BASS_block(d2,m),dtype=object)
	...
	for i in np.arange(M):
		...
		if r1 is not None:
			...
			r1set=None
		r2set = np.concatenate(new_r2blocks)
		wp_t = wpmu_d1d2(d1=d1set, d2=d2set, r1=r1set, r2=r2set, sbins=sbins, mumax=mumax,mubins=mubins, estimator=estimator)
		wp_arr=np.vstack((wp_arr,wp_t))
	wp_arr=wp_arr[1:]
